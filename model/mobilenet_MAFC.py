...
class MobileNetV1(torch.nn.Module):
    ...
    def __init__(self,
                 ...
                 in_channels=3,
                 in_size=(224, 224),
                 use_data_batchnorm=True,pool_types=None):
        super().__init__()
        ...
        for stage_id, stage_channels in enumerate(channels):
            ...
            for unit_id, unit_channels in enumerate(stage_channels):
                ...
                stage.add_module("unit{}".format(unit_id + 1), DepthwiseSeparableConvBlock(in_channels=in_channels, out_channels=unit_channels, stride=stride,pool_types=pool_types))
    ...
    def __init__(self,
        ...
        for stage_id, stage_channels in enumerate(channels):
            ...
            for unit_id, unit_channels in enumerate(stage_channels):
                ...
                stage.add_module("unit{}".format(unit_id + 1), LinearBottleneck(in_channels=in_channels, mid_channels=mid_channel, out_channels=unit_channels, stride=stride,use_se=True,pool_types=pool_types))
                in_channels = unit_channels
            self.backbone.add_module("stage{}".format(stage_id + 1), stage)
    ...
    def __init__(self,
        ...
        for stage_id, stage_channels in enumerate(channels):
            ...
            for unit_id, unit_channels in enumerate(stage_channels):
                ...
                stage.add_module("unit{}".format(unit_id + 1), LinearBottleneck(in_channels=in_channels, mid_channels=mid_channel, out_channels=unit_channels, stride=stride, activation=activation, kernel_size=kernel_size,use_se=use_se,pool_types=pool_types))
                in_channels = unit_channels
            self.backbone.add_module("stage{}".format(stage_id + 1), stage)
        ...
        if final_conv_se:
            self.backbone.add_module("final_se", ECA(in_channels))
...
def build_mobilenet_v1(num_classes, width_multiplier=1.0, cifar=False,pool_types=None):
